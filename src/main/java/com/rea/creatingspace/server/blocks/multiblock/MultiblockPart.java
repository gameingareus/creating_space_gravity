...
public interface MultiblockPart extends IWrenchable {
	...
	public static interface MultiblockMainPart extends MultiblockPart {
		...
		public static enum MultiblockPartType {
			BLANK(() -> BlockInit.GHOST_BLOCK.get().defaultBlockState()),
			MAIN(),
			KINETIC(f -> BlockInit.KINETIC_INPUT.getDefaultState().setValue(KineticInputBlock.SHAFT_FACING, f)),
			...
			private MultiblockPartType(IOBlockType type) {
				this(() -> BlockInit.IO_BLOCK.get().defaultBlockState().setValue(IOBlock.TYPE, type));
			}
